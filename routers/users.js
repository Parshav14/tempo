...
router.post("/add_client", auth, async(req, res) => {
    try {
        ...
        let query = `INSERT INTO tbl_client (customer, first_name, last_name, email, mobile, country, state, city, zipcode, address) VALUE 
                    ('${customer}', '${first_name}', '${last_name}', '${email}', '${mobile}', '${country}', '${state}', '${city}', '${zipcode}', '${address}')`
        await mySqlQury(query)
...
router.post("/edit_client/:id", auth, async(req, res) => {
    try {
        ...
        let query = `UPDATE tbl_client SET customer = '${customer}', first_name = '${first_name}', last_name = '${last_name}', email = '${email}', mobile = '${mobile}', country = '${cuuntry}', state = '${state}', city = '${city}', zipcode = '${zipcode}', address = '${address}' WHERE id = '${req.params.id}'`
        await mySqlQury(query)
