...
router.get("/", auth, async (req, res) => {
    try {
        ...
        const shipment_notification = await mySqlQury(
        ...
        let customer_data = await mySqlQury(
          `SELECT * FROM tbl_customers WHERE login_id = '${role_data.id}'`
        );
        let drivers_data = await mySqlQury(
          `SELECT * FROM tbl_drivers WHERE login_id = '${role_data.id}'`
        );
        ...
        if (role_data.role == '1') {
            ...
            res.render("index", {
              ...
              registered_packages:
                packages_total == null ? 0 : packages_total.toFixed(2),
              shipments:
                shipments_total == null ? 0 : shipments_total.toFixed(2),
              pickups: pickups_total == null ? 0 : pickups_total.toFixed(2),
              consolidated:
                consolidated_total == null ? 0 : consolidated_total.toFixed(2),
            ...
            res.render("index", {
              ...
              logistics_service_data: 0,
                ...
                console.log("Unknown role:", role_data.role);
                req.flash("errors", "Invalid user role");
                return res.redirect("/");
  ...
  router.get("/profile", auth, async (req, res) => {
    try {
      ...
      const data = await mySqlQury(
        `SELECT * FROM tbl_admin WHERE id = '${role_data.id}'`
      );
