...
router.get("/add_consolidated", auth, async(req, res) => {
    try {
        ...
        const shipment_data = await mySqlQury(`SELECT * FROM tbl_consolidated ORDER BY ID DESC LIMIT 1`)
        ...
        if (shipment_data == "") {
            ...
            if (shipment_data[0].id < 100) {
                let invoice = '0000' + (shipment_data[0].id + 1)
            ...
            }else if (shipment_data[0].id > 100) {
                let invoice = '000' + (shipment_data[0].id + 1)
            ...
            }else if (shipment_data[0].id > 1000) {
                let invoice = '00' + (shipment_data[0].id + 1)
            ...
            }else if (shipment_data[0].id > 10000) {
                let invoice = '0' + (shipment_data[0].id + 1)
                ...
                let invoice = (shipment_data[0].id + 1)
...
router.get("/find_shipment/ajax", auth, async(req, res) => {
    try {
        const shipment_list = await mySqlQury(`SELECT tbl_shipment.*, (select tbl_shipping_status.status_name from tbl_shipping_status where tbl_shipment.delivery_status = tbl_shipping_status.id) as shipping_status FROM tbl_shipment WHERE delivery_status = '3'`)
...
router.post("/add_consolidated", auth, upload.single('image'), async(req, res) => {
    ...
    host: email_data[0].email_host, // Replace with your SMTP server's hostname
    port: email_data[0].email_port, // Replace with the appropriate port (465 for secure SSL/TLS)
    secure: true, // Use SSL/TLS
    auth: {
        user: email_data[0].email, // Replace with your email
        pass: email_data[0].email_password // Replace with your email password
    }
});
